...
async def moderate_briefing(client: genai.Client, briefing: str) -> str | None:
    ...
    response = await client.aio.models.generate_content(
        model=SUMMARY_MODEL,
        contents=(
            "You are a strict content moderation filter for a phone calling service. "
            "A user has submitted a briefing for an AI agent to call someone on their behalf.\n\n"
            "This service is ONLY for practical tasks like calling businesses and services. "
            "It is NOT for delivering personal messages to people.\n\n"
            ...
            "- Delivering sensitive or distressing news (death, illness, breakups, etc.)\n"
            "- Tracking or checking someone's location or whereabouts\n"
            "- Arranging pickup or activities involving minors/children\n"
            "- Delivering personal emotional messages (love confessions, apologies, etc.) — the caller should do this themselves\n"
            "- Pranks, jokes, or trick calls\n"
            "- Any call where the real intent could be manipulation, deception, or causing distress\n\n"
            "ALLOW practical tasks like: booking reservations, asking about business hours, "
            "scheduling appointments, checking availability, making inquiries, cancellations, "
            "calling service providers, asking about prices, etc.\n\n"
            "When in doubt, REJECT. It is better to be too strict than to allow harmful calls.\n\n"
            f"BRIEFING:\n{briefing}\n\n"
